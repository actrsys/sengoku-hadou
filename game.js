...
class GameSystem {
    ...
    static calcWarDamage(atkStats, defStats, atkSoldiers, defSoldiers, defWall, type) {
        ...
        // 基本攻撃力 (兵士数スケール)
        const atkBase = (atkStats.ldr * 1.2) + (atkStats.str * 0.3) + (atkSoldiers * CONFIG.War.SoldierPower);
        const defBase = (defStats.ldr * 1.0) + (defStats.int * 0.5) + (defSoldiers * CONFIG.War.SoldierPower);

        // 城防御は城攻め以外で有効
        const wallEffect = defWall * CONFIG.War.WallDefense;

        let multiplier = 1.0;
        let soldierRate = 1.0; // 兵士へのダメージ比率
        let wallRate = 0.0;    // 城壁へのダメージ比率
        let counterRisk = 1.0; // 反撃被ダメージ倍率 (1.0 = 標準)

        switch(type) {
            case 'bow': // 兵士のみ、低威力、低リスク
                multiplier = 0.6; soldierRate = 1.0; wallRate = 0.0; counterRisk = 0.5; break;
            case 'siege': // 城壁特化、兵士微小、中リスク
                multiplier = 1.0; soldierRate = 0.05; wallRate = 3.0; counterRisk = 1.0; break;
            case 'charge': // 全力、両方ダメージ、高リスク
                multiplier = 1.2; soldierRate = 1.0; wallRate = 0.5; counterRisk = 1.5; break;
            case 'def_bow': // 防御: 兵士削り
                multiplier = 0.5; soldierRate = 1.0; wallRate = 0.0; break;
            case 'def_attack': // 防御: 通常
                multiplier = 1.0; soldierRate = 1.0; wallRate = 0.0; break;
            case 'def_charge': // 防御: 全力反撃
                multiplier = 1.5; soldierRate = 1.0; wallRate = 0.0; break;
        }

        // ダメージ計算: 攻撃 / (攻撃 + 防御)
        const ratio = atkPower / (atkPower + defBase + (type !== 'siege' ? wallEffect : 0));
        let baseDmg = atkPower * ratio * multiplier * rand;
        baseDmg = Math.max(50, baseDmg);

        return {
            soldierDmg: Math.floor(baseDmg * soldierRate),
            wallDmg: Math.floor(baseDmg * wallRate * 0.5), // 城壁はHPが低いので補正
            risk: counterRisk
        };
    }
    ...
    // 謀略 (兵士ダメージ)
    static calcScheme(atkBusho, defBusho) {
        const atkInt = atkBusho.intelligence; const defInt = defBusho ? defBusho.intelligence : 30;
        const successRate = (atkInt / (defInt + 20)) * 0.6; // 少し厳しく
        if (Math.random() > successRate) return { success: false, damage: 0 };
        return { success: true, damage: Math.floor(atkInt * 10 * (Math.random() + 0.5)) }; // 数百〜千程度
    }
    ...
    static calcFire(atkBusho, defBusho) {
        ...
        if (Math.random() > successRate) return { success: false, damage: 0 };
        return { success: true, damage: Math.floor(atkInt * 3 * (Math.random() + 0.5)) }; // 数百程度
    }
...
class UIManager {
    constructor(game) {
        ...
        this.selectorHeader = document.querySelectorAll('.selector-header-row'); // Multiple headers
    ...
    openBushoSelector(actionType, targetId = null, extraData = null) { /* 省略なし */ if (actionType === 'appoint') { const isDaimyoHere = this.game.getCastleBushos(this.currentCastle.id).some(b => b.isDaimyo); if (isDaimyoHere) { alert("大名の居城は城主を変更できません"); return; } } this.selectorModal.classList.remove('hidden'); document.getElementById('selector-title').textContent = "武将を選択"; this.selectorList.innerHTML = ''; const contextEl = document.getElementById('selector-context-info'); contextEl.classList.remove('hidden'); const c = this.currentCastle; let infoHtml = ""; let sortKey = 'strength'; let sortLabel = "武力"; let bushos = []; let isMulti = false;
        ...
        else if (actionType === 'employ_doer') { bushos = this.game.getCastleBushos(c.id).filter(b => b.status !== 'ronin'); infoHtml = "<div>登用を行う担当官を選択してください</div>"; sortKey = 'charm'; sortLabel = '魅力'; } 
        else if (actionType === 'diplomacy_doer') { bushos = this.game.getCastleBushos(c.id).filter(b => b.status !== 'ronin'); infoHtml = "<div>外交の担当官を選択してください</div>"; sortKey = 'diplomacy'; sortLabel = '外交'; } 
        else {
        ...
        bushos.forEach(b => {
            ...
            div.innerHTML = `<input type="${inputType}" name="sel_busho" value="${b.id}" ${!isSelectable ? 'disabled' : ''}><span class="col-act">${b.isActionDone?'[済]':'[可]'}</span><span class="col-name">${b.name}</span><span class="col-rank">${b.getRankName()}</span><span class="col-stat">${b.leadership}/${b.strength}/${b.politics}/${b.diplomacy}/${b.intelligence}/${b.charm}</span>`;
    ...
    showCastleInfo(castle) {
        this.bushoDetailModal.classList.remove('hidden'); document.getElementById('busho-modal-title').textContent = "城情報";
        ...
        if (isVisible) {
            infoArea.innerHTML = `<h3>${castle.name} (${clanData ? clanData.name : '中立'})</h3>
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:5px; font-size:0.9rem;">
                <div>兵士: ${castle.soldiers}</div><div>防御: ${castle.defense}/${castle.maxDefense}</div>
                <div>石高: ${castle.kokudaka}/${castle.maxKokudaka}</div><div>商業: ${castle.commerce}/${castle.maxCommerce}</div>
                <div>民忠: ${castle.loyalty}</div><div>人口: ${castle.population}</div>
            </div>`;
            this.renderBushoList(castle.id, this.bushoDetailList);
        } else {
            infoArea.innerHTML = `<h3>${castle.name} (${clanData ? clanData.name : '中立'})</h3><p>情報は不明です</p><button class="btn-primary" onclick="window.GameApp.ui.investigateFromInfo(${castle.id})">調査する</button>`;
            this.bushoDetailList.innerHTML = "";
        }
    }
    investigateFromInfo(castleId) {
        this.bushoDetailModal.classList.add('hidden');
        this.game.enterMapSelection('investigate');
        // 自動でターゲット選択まで進めるとなお良いが、とりあえずモード切替のみ
    }
    // onclick helper
    showBushoListById(castleId) { this.showCastleInfo(this.game.getCastle(castleId)); } 
    showBushoList(castle) {
        this.bushoDetailModal.classList.remove('hidden'); document.getElementById('busho-modal-title').textContent = `${castle.name} 所属武将`;
        document.getElementById('castle-detail-info').classList.add('hidden'); // リストのみモードなら城情報は隠す
        this.renderBushoList(castle.id, this.bushoDetailList);
    }
...
class GameManager {
    ...
    processTurn() { if (this.currentIndex >= this.turnQueue.length) { this.endMonth(); return; } const castle = this.turnQueue[this.currentIndex]; this.ui.renderMap(); if (castle.ownerClan === this.playerClanId) { this.isProcessingAI = false; this.ui.log(`【${castle.name}】命令を下してください`); this.ui.showControlPanel(castle); } else { this.isProcessingAI = true; this.ui.log(`【${castle.name}】(他国) 思考中...`); document.getElementById('control-panel').classList.add('hidden'); setTimeout(() => this.execAI(castle), 600); } }
    ...
    // ... Map/Command logic (Same structure, added AI error handling & Fire) ...
    ...
    execAI(castle) {
        try {
            ...
            if (castellan && !castellan.isActionDone) {
                ...
                if(enemies.length > 0 && castle.soldiers > 3000) {
                     this.startWar(castle, enemies[0], [castellan], Math.floor(castle.soldiers*0.5));
                } else {
    ...
    resolveWarAction(type) {
        ...
        if (type === 'scheme') {
            ...
            else { target.soldiers = Math.max(0, target.soldiers - result.damage); if (s.isPlayerInvolved) this.ui.log(`R${s.round} 謀略成功！ 兵士に${result.damage}の被害`); }
            this.advanceWarTurn(); return;
        }
        if (type === 'fire') {
            const actor = isAtkTurn ? s.atkBushos[0] : s.defBusho; const targetBusho = isAtkTurn ? s.defBusho : s.atkBushos[0];
            const result = GameSystem.calcFire(actor, targetBusho);
            if (!result.success) { if (s.isPlayerInvolved) this.ui.log(`R${s.round} 火攻失敗！`); }
            else { 
                if(isAtkTurn) s.defender.defense = Math.max(0, s.defender.defense - result.damage); 
                // 攻撃側には城防御がないので兵士微減とする
                else target.soldiers = Math.max(0, target.soldiers - 50);
                if (s.isPlayerInvolved) this.ui.log(`R${s.round} 火攻成功！ 防御に${result.damage}の被害`);
            }
            this.advanceWarTurn(); return;
        }
        ...
        // リスク反撃 (簡易)
        if(result.risk > 1.0) {
            const counterDmg = Math.floor(actualSoldierDmg * (result.risk - 1.0) * 0.5);
            const actorArmy = isAtkTurn ? s.attacker : s.defender;
            actorArmy.soldiers = Math.max(0, actorArmy.soldiers - counterDmg);
            if(s.isPlayerInvolved) this.ui.log(`(反撃被害: ${counterDmg})`);
        }

        if (s.isPlayerInvolved) {
            let actionName = type.includes('bow') ? "弓攻撃" : type.includes('siege') ? "城攻め" : "力攻め";
            if (type.includes('def_')) actionName = type === 'def_bow' ? "弓反撃" : type === 'def_charge' ? "全力反撃" : "反撃";
