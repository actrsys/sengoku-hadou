...
class GameSystem {
    ...
    static getBestStat(bushos, type) { if (!bushos || bushos.length === 0) return 30; let max = 0; bushos.forEach(b => { let val = type==='str'?b.strength : type==='int'?b.intelligence : type==='charm'?b.charm : type==='dip'?b.diplomacy : b.politics; if(val>max) max=val; }); return max; }
    ...
    static calcWarDamage(atkStats, defStats, atkSoldiers, defSoldiers, defWall, type) {
        ...
        switch(type) {
            case 'bow': multiplier = 0.6; soldierRate = 1.0; wallRate = 0.0; counterRisk = 0.5; break;
...
class UIManager {
    constructor(game) {
        ...
        this.selectorHeader = document.querySelector('.selector-content .grid-table-header'); 
    ...
    showGunshiAdvice(action, onConfirm) {
        // 失敗判定のないものは助言なし
        if (['farm','commerce','repair','draft','charity','transport','appoint_gunshi','appoint','banish'].includes(action.type)) { onConfirm(); return; }
    ...
    openBushoSelector(actionType, targetId = null, extraData = null) {
        ...
        const contextEl = document.getElementById('selector-context-info'); contextEl.classList.remove('hidden'); const c = this.currentCastle; let infoHtml = ""; let sortKey = 'strength';
        let bushos = []; let isMulti = false;
        
        if (actionType === 'appoint_gunshi') { bushos = this.game.bushos.filter(b => b.clan === this.game.playerClanId && !b.isDaimyo && !b.isCastellan && b.status !== 'ronin' && b.status !== 'dead'); infoHtml = "<div>軍師に任命する武将を選択してください</div>"; sortKey = 'intelligence'; } 
            ...
            if (actionType === 'farm') { infoHtml = `<div>金: ${c.gold} (1回500)</div>`; sortKey = 'politics'; }
            else if (actionType === 'commerce') { infoHtml = `<div>金: ${c.gold} (1回500)</div>`; sortKey = 'politics'; }
            ...
            else if (actionType === 'war_deploy') { sortKey = 'strength'; isMulti = true; }
            else if (actionType === 'move_deploy') { sortKey = 'strength'; isMulti = true; }
            else if (actionType === 'scheme_select') { sortKey = 'intelligence'; }
            else if (actionType === 'appoint') { sortKey = 'leadership'; }
            else if (actionType === 'investigate_deploy') { sortKey = 'strength'; }
        }
    ...
    openQuantitySelector(type, data, targetId) {
        ...
        this.charityTypeSelector.classList.add('hidden'); const c = this.currentCastle;
        ...
        if (type === 'draft') {
            ...
            this.quantityConfirmBtn.onclick = () => { const val = parseInt(inputs.gold.num.value); if(val <= 0) return; this.quantityModal.classList.add('hidden'); this.game.executeDraft(data, val); };
            ...
            this.quantityConfirmBtn.onclick = () => { const charityType = document.querySelector('input[name="charityType"]:checked').value; this.quantityModal.classList.add('hidden'); this.game.executeCharity(data, charityType); };
    ...
    showCastleInfo(castle) {
        ...
        if (isVisible) {
            infoArea.innerHTML = `<h3>${castle.name} (${clanData ? clanData.name : '中立'})</h3><div style="display:grid; grid-template-columns:1fr 1fr; gap:5px; font-size:0.9rem;"><div>兵士: ${castle.soldiers}</div><div>防御: ${castle.defense}/${castle.maxDefense}</div><div>石高: ${castle.kokudaka}/${castle.maxKokudaka}</div><div>商業: ${castle.commerce}/${castle.maxCommerce}</div><div>民忠: ${castle.loyalty}</div><div>人口: ${castle.population}</div></div>`;
            this.renderBushoList(castle.id, body);
        } else {
            infoArea.innerHTML = `<h3>${castle.name} (${clanData ? clanData.name : '中立'})</h3><p class="panel-msg">情報は不明です</p><button class="btn-primary" onclick="window.GameApp.ui.investigateFromInfo(${castle.id})">調査する</button>`; body.innerHTML = "";
        }
    }
    ...
    showBushoList(castle) {
        const modal = document.getElementById('busho-detail-modal'); const body = document.getElementById('busho-detail-list'); modal.classList.remove('hidden'); document.getElementById('busho-modal-title').textContent = `${castle.name} 所属武将`; document.getElementById('castle-detail-info').classList.add('hidden');
        this.renderBushoList(castle.id, body);
    }
...
class GameManager {
    ...
    processTurn() {
        ...
        if (castle.ownerClan === this.playerClanId) {
            this.isProcessingAI = false; this.ui.renderMap(); // Ensure AI guard is removed
            this.ui.log(`【${castle.name}】命令を下してください`); this.ui.showControlPanel(castle);
        } else {
            this.isProcessingAI = true; this.ui.renderMap(); // Ensure AI guard is active
    ...
    enterMapSelection(actionType, preSelectedTargetId=null) {
        ...
        else if (actionType === 'transport' || actionType === 'move') { this.validTargets = this.castles.filter(c => c.ownerClan === current.ownerClan && c.id !== current.id && GameSystem.isAdjacent(current, c)); }
        else if (actionType === 'investigate') { this.validTargets = this.castles.filter(c => c.ownerClan !== 0 && c.ownerClan !== current.ownerClan); }
    ...
    execAI(castle) {
        try {
            ...
            if (castellan && !castellan.isActionDone) {
                ...
                if(validEnemies.length > 0 && castle.soldiers > 3000) {
                     this.startWar(castle, validEnemies[0], [castellan], Math.floor(castle.soldiers*0.5));
                } else {
                     if(castle.gold > 500) { const val = GameSystem.calcDevelopment(castellan); castle.kokudaka+=val; castle.gold-=500; }
