...
class DiplomacyManager {
    ...
    /**
     * ★今回引っ越し：毎月1回、大名家として外交の狙いを1つに絞って記憶する魔法です！
     * ai_operation.js から引越しし、軍事AIのスコアと連動した遠交近攻ロジックを搭載しています。
     */
    planMonthlyDiplomacy(clan) {
        // 一旦、今までの記憶を忘れます
        ...
        // ★敵対勢力の後ろ（向こう隣）にいる、挟み撃ちの相方候補を記憶するメモ帳
        const strategicPartnerClans = new Set();

        directNeighbors.forEach(neighborId => {
            const rel = this.getRelation(myClanId, neighborId);
                    ...
                    if (enemyCastle.adjacentCastleIds) {
                        enemyCastle.adjacentCastleIds.forEach(adjId => {
                            ...
                            if (adjCastle && adjCastle.ownerClan !== 0 && adjCastle.ownerClan !== myClanId && adjCastle.ownerClan !== neighborId) {
                                ...
                                strategicPartnerClans.add(adjCastle.ownerClan);
                            }
                        });
                    }
                });
        ...
        const allyCount = this.getAllyCount(myClanId);
        const enemyThreats = [];
        uniqueNeighbors.forEach(targetClanId => {
            const rel = this.getRelation(myClanId, targetClanId);
            const isProtected = rel && this.isNonAggression(rel.status);
            if (!isProtected) {
        ...
        const diplomacyTargets = this.getDiplomacyPriorityList(myClanId, uniqueNeighbors, mainThreatId);

        // ★軍事部（ai.js）の点数計算をシミュレーションして、攻撃したさのスコアを調べます
        const attackScores = {};
        myCastles.forEach(myCastle => {
            const enemies = [];
            if (myCastle.adjacentCastleIds) {
                myCastle.adjacentCastleIds.forEach(adjId => {
                    const adjCastle = this.game.getCastle(adjId);
                    if (adjCastle && adjCastle.ownerClan !== myClanId && adjCastle.ownerClan !== 0) {
                        enemies.push(adjCastle);
                    }
                });
            }
            if (enemies.length > 0) {
                ...
                if (myGeneral) {
                    const decision = this.game.aiEngine.decideAttackTarget(myCastle, myGeneral, enemies);
                    if (decision && decision.score > 0 && !decision.target.isKunishuTarget) {
                        const tClanId = decision.target.ownerClan;
                        if (tClanId > 0) {
                            if (!attackScores[tClanId] || decision.score > attackScores[tClanId]) {
                                attackScores[tClanId] = decision.score; // 一番高い攻撃意欲スコアをメモ
                            }
                        }
                    }
                }
        ...
        // 外交リストの各勢力に「後ろを突く相方か」の正確なフラグを立て直します
        diplomacyTargets.forEach(tData => {
            if (strategicPartnerClans.has(tData.clanId)) {
                tData.isStrategicPartner = true;
            }
        });

        // 元の優先順位（義理など）をベースにしつつ、新しい戦略スコアをブレンドして並べ替えます
        diplomacyTargets.forEach((tData, index) => {
            let strategicScore = (diplomacyTargets.length - index) * 20;

            if (tData.isStrategicPartner) {
                strategicScore += (100 - myDaimyo.duty) * 1.5 + 40;
            }

            strategicScore -= (attackScores[tData.clanId] || 0);
            tData.combinedStrategicScore = strategicScore;
        });

        diplomacyTargets.sort((a, b) => b.combinedStrategicScore - a.combinedStrategicScore);
        ...
        for (let targetData of diplomacyTargets) {
            ...
            let decision = this.determineAIDiplomacyAction(
            ...
            const currentRel = this.getRelation(myClanId, targetClanId);

            // 現在「敵対」しているけれど、軍事的な攻撃スコアが極めて低い（15点未満）相手には和睦を提案します
            if (currentRel && currentRel.status === '敵対') {
                const myAttackScore = attackScores[targetClanId] || 0;
                if (myAttackScore < 15) {
                    decision.action = 'truce';
                    decision.gold = 0;
                }
            }

            // 敵の背後を突く「挟み撃ちの相方」に対して同盟を促します（身内への上書きガード付き）
            const isAlreadyPartner = currentRel && ['同盟', '支配', '従属'].includes(currentRel.status);
            if (targetData.isStrategicPartner && !isAlreadyPartner && ['none', 'goodwill'].includes(decision.action)) {
                decision.action = 'alliance';
                decision.gold = 0;
            }
