...
class GameSystem {
    ...
    static calcTraining(busho) { const base = 1 + (busho.leadership * 0.05 + busho.strength * 0.025); return this.applyVariance(base, 0.2); }
    static calcSoldierCharity(busho) { const base = 0 + (busho.leadership * 0.05) + (busho.charm * 0.025); return this.applyVariance(base, 0.2); }
    static calcDraftFromGold(gold, busho, castlePopulation) { const bonus = 1.0 + ((busho.leadership + busho.strength + busho.charm) / 300) * (1.1 - 1.0); const popBonus = 1.0 + (castlePopulation * 0.00001); return Math.floor(gold * 1.0 * bonus * popBonus); }
