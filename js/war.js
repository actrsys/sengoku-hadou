...
class WarManager {
    ...
    startWar(atkCastle, defCastle, atkBushos, atkSoldierCount) {
        ...
        } catch(e) { console.error("StartWar Error:", e); this.game.finishTurn(); }
    }
    ...
    execWarAI() { 
        ...
        // 撤退判断 (知略が高い、または難易度が高いと損切りが早い)
        if (isDefender) {
            const dangerRatio = s.defender.soldiers / (s.attacker.soldiers + 1);
            let retreatThreshold = 0.2 + (smartness * 0.2); // 賢いほど早め(0.4)に撤退を考える
        ...
        const options = [];
        if (isDefender) {
            options.push('def_charge', 'def_bow', 'def_attack');
        } else {
            options.push('charge', 'bow', 'siege');
        }
        // 計略系は確率で候補入り
        ...
        options.forEach(cmd => {
            ...
            if (cmd === 'siege') {
                // 城壁へのダメージ重視
                score += (multiplier * 50) + (enemyDefense > 0 ? 100 : 0);
            } else if (cmd === 'def_attack') {
                // 籠城は生存重視
                score += (s.defender.soldiers < s.attacker.soldiers) ? 200 : -100;
            } else {
                // 兵士へのダメージ重視
    ...
    endWar(attackerWon, isRetreat = false, capturedInRetreat = [], retreatTargetId = null) { 
            const s = this.state; 
            s.active = false; // 合戦終了フラグを即座に落とす
            
            if (s.isPlayerInvolved) { 
                const warModal = document.getElementById('war-modal'); 
                if(warModal) warModal.classList.add('hidden');
                this.game.isProcessingAI = false; // プレイヤー関与時はUIロックを解除
            }
            ...
            if (isRetreat) {
                ...
                } else if (s.defender.ownerClan === this.game.playerClanId) {
                    this.game.ui.renderMap();
                    this.game.ui.showControlPanel(this.game.getCastle(s.defender.id));
                } else {
                    this.game.finishTurn();
                }
                return;
            ...
            // 【重要】手番終了の振り分け
            if (s.attacker.ownerClan === this.game.playerClanId) {
                // プレイヤーが攻撃した場合は、その拠点のターンを終了
                this.game.ui.renderMap();
                this.game.finishTurn(); 
            } else if (s.defender.ownerClan === this.game.playerClanId) {
                // 防衛した場合は、自分の操作に戻す（finishTurnを呼ばない）
                this.game.ui.renderMap();
                this.game.ui.showControlPanel(this.game.getCastle(s.defender.id));
            } else {
                // AI同士なら普通に次のAIへ
