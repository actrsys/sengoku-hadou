...
class WarManager {
    ...
    /**
     * 兵力配分の互換窓口。計算本体は TroopAllocationService に一元化する。
     */
    autoDivideSoldiers(bushos, totalSoldiers, totalHorses = 0, totalGuns = 0, isSeaBattleParam = false, isPlayerUI = false) {
        ...
        return TroopAllocationService.autoDivideSoldiers({
            bushos,
            totalSoldiers,
            totalHorses,
            totalGuns,
    ...
    getAvailableCommands(isAtkTurn) {
        ...
        if (s.turn.startsWith('attacker')) {
            ...
            if (s.turn === 'defender' && s.defender.ownerClan !== 0 && this.game.getClanCastles(s.defender.ownerClan).some(c => c.id !== s.defender.id && MapGraphService.isReachable(this.game, s.defender, c, s.defender.ownerClan))) commands.push({ label: "撤退", type: "retreat" });
        }
        return commands;
    }
    ...
    resolveAutoFieldWar() {
        ...
        const consumeRate = (window.WarParams.War.RiceConsumptionAtk) * 0.5;
    ...
    execWarAI() { 
        ...
        if (isDefenderTurn && s.turn === 'defender' && s.defender.ownerClan !== 0 && !s.defender.isKunishu && this.game.getClanCastles(s.defender.ownerClan).some(c => c.id !== s.defender.id && MapGraphService.isReachable(this.game, s.defender, c, s.defender.ownerClan))) {
        ...
        if (!isDefenderTurn) {
            ...
            if (s.defDamageHistory && s.defDamageHistory.length >= 5) {
                ...
                let maxRounds = window.WarParams.Military.WarMaxRounds;
