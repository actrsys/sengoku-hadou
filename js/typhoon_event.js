...
window.GameEvents.push({
    ...
    execute: async function(game) {
        ...
        if (window.ProvinceImageDataCache) {
            ...
            // ① 左端に集中させる魔法！（サイコロを3乗して、左側に加速度的に偏らせます）
            let r = Math.pow(Math.random(), 3); 
            // 左(-500) から 右(widthの70%くらい) の範囲ですが、圧倒的に左寄りで発生します
            ...
            while (typhoonX < width + typhoonRadius && typhoonY > -typhoonRadius && typhoonRadius > 30) {
                ...
                // ② 陸地にあたった時の「角度（進行方向）」の加速度的な減衰！
                if (wasOnLand) {
                    // 陸地に長くいるほど、上に進む力が急激に失われます（最小で元の10%まで落ちる）
                    let yMultiplier = Math.max(0.1, 0.5 - (landCount * 0.05));
                    moveY *= yMultiplier; 
                    moveX *= 0.8; // スピード全体も落ちる
                }
