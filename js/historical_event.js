...
window.EventCheck = {
    ...
    // ⑤ ★「AND（かつ）」の動き：リストの「全員」が大名として存在しているか確認します
        ...
        if (isAttack) {
            ...
            for (const busho of deadOdaBushos) {
                ...
                if (isPlayerInvolved) {
                    ...
                    // 自分が織田家なら「当家」、それ以外（今川家）なら武将のデータから作った家名を表示させます
                    const prefix = (game.playerClanId === nobunaga.clan) ? "当家" : odaName;
            ...
            for (const busho of deadImagawaBushos) {
                ...
                if (isPlayerInvolved) {
                    const bushoName = busho.name.replace('|', '');
                    // 自分が今川家なら「当家」、それ以外（織田家）なら武将のデータから作った家名を表示させます
                    const prefix = (game.playerClanId === imagawaClanId) ? "当家" : imagawaName;
                    const msg = `${prefix}の${bushoName}が討死しました。`;
