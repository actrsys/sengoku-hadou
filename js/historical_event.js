...
window.GameEvents.push({
    ...
    checkCondition: function(game, context) {
        console.log("◆【調査】桶狭間２の条件チェックが呼ばれました！");
        
        // 戦闘システムからコンテキスト（状況データ）が届いていなければストップします
        if (!context) {
            console.log("× エラー：context(状況データ)が届いていません");
            return false;
        }

        // H. 攻撃目標が名古屋城（ID: 11）か確認します
        if (!context.defender || context.defender.id !== 11) {
            console.log("× エラー：攻撃目標が名古屋城ではありません。");
            return false;
        }
        ...
        if (!hasYoshimoto) {
            console.log("× エラー：攻撃側に今川義元がいません");
            return false;
        }

        // A. 義元が大名であるか確認します
        const yoshimoto = game.getBusho(1004001);
        if (!yoshimoto || !yoshimoto.isDaimyo) {
            console.log("× エラー：今川義元が大名ではありません");
            return false;
        }

        // D, F. 信長が大名で、清州城（ID: 7）にいるか確認します
        const nobunaga = game.getBusho(1006001);
        if (!nobunaga || !nobunaga.isDaimyo || nobunaga.castleId !== 7) {
            console.log("× エラー：織田信長が清州城にいない、または大名ではありません");
            return false;
        }
        ...
        if (!kiyosu || kiyosu.ownerClan !== odaClanId) {
            console.log("× エラー：清州城が織田家のものではありません");
            return false;
        }
        if (!nagoya || nagoya.ownerClan !== odaClanId) {
            console.log("× エラー：名古屋城が織田家のものではありません");
            return false;
        }

        // G. 松平元康が城主として存在するか確認します
        const motoyasu = game.getBusho(1004004);
        if (!motoyasu || !motoyasu.isCastellan) {
            console.log("× エラー：松平元康が城主ではありません");
            return false;
        }

        console.log("〇 大成功！桶狭間２の条件をすべてクリアしました！");
        return true;
    },
