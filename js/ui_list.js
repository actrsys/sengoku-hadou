...
class UIListManager {
    ...
    _renderCurrentModal() {
        ...
        // ★修復＆更新：消えていた案内板をすべて復活させ、拠点(kyoten_list, castle_detail)を新居宛てに書き換えました！
        ...
        if (listContainer) {
            ...
            if (btnKunishu && kunishuCount > 0) {
                btnKunishu.onclick = (e) => {
                    e.stopPropagation();
                    if (window.AudioManager) window.AudioManager.playSE('decision.ogg');
                    // ★連絡先：諸勢力はまだ旧居にいるのでinfoを呼びます
                    this.ui.info.showKunishuList(kunishus, castle);
                };
            }
    ...
    _renderKyotenList(clanId, scrollPos = 0) {
        ...
        if (this.kyotenSavedSortedCastles && this.kyotenLastSortStateKey === currentSortStateKey) {
            ...
            displayCastles = this._prepareStableSortBase('kyoten', this.kyotenSavedCastles, this.currentKyotenSortKey);

            if (!this.currentKyotenSortKey) {
                displayCastles.sort((a, b) => (a.sortNo || 0) - (b.sortNo || 0));
                this._saveStableSortResult('kyoten', null);
            } else {
                ...
                this._saveStableSortResult('kyoten', displayCastles);
            }
        ...
        displayCastles.forEach(c => {
            ...
            items.push({
                // ★連絡先修正：HTMLからの呼び出しを新居(list)宛てに変更
                onClick: `window.GameApp.ui.list.showCastleDetail(${c.id})`,
                cells: cells
            });
        });
