 ...
 * 修正: 操作不能バグ修正(pointer-events対応)、大将アイコン修正(凸+★)
 */
...
class FieldWarManager {
    ...
    startFieldWar(warState, onComplete) {
        ...
        let atkX = 5, defX = 25;
        if (isDefPlayer && !isAtkPlayer) { atkX = 25; defX = 5; }

        this.units = [];
        const yPositions = [10, 8, 12, 6, 14];

        if (warState.atkAssignments) {
            warState.atkAssignments.forEach((assign, index) => {
                if (assign.soldiers <= 0) return;
                this.units.push({
                    id: `atk_${index}`, name: assign.busho.name,
                    isAttacker: true, isPlayer: isAtkPlayer, isGeneral: index === 0,
                    x: atkX, y: yPositions[index % 5], direction: isAtkPlayer ? 1 : 4,
                    mobility: 4, ap: 4, soldiers: assign.soldiers,
                    stats: WarSystem.calcUnitStats([assign.busho]), hasActionDone: false
                });
            });
        }
        ...
        if (warState.defAssignments) {
            warState.defAssignments.forEach((assign, index) => {
                if (assign.soldiers <= 0) return;
                this.units.push({
                    id: `def_${index}`, name: assign.busho.name,
                    isAttacker: false, isPlayer: isDefPlayer, isGeneral: index === 0,
                    x: defX, y: yPositions[index % 5], direction: isDefPlayer ? 1 : 4,
                    mobility: 4, ap: 4, soldiers: assign.soldiers,
                    stats: WarSystem.calcUnitStats([assign.busho]), hasActionDone: false
                });
            });
    ...
    initUI() {
        ...
        if (this.mapEl) {
            ...
            this.mapEl.oncontextmenu = (e) => { e.preventDefault(); this.cancelAction(); };
        }
    ...
    cancelAction() {
        ...
        if (this.state === 'PHASE_MOVE' && this.turnBackup && 
            ...
            if (this.previewTarget) { this.previewTarget = null; this.updateMap(); }
            return;
        }

        if (this.turnBackup) {
            unit.x = this.turnBackup.x; unit.y = this.turnBackup.y;
            unit.direction = this.turnBackup.direction; unit.ap = this.turnBackup.ap;
            ...
            for (let row = 0; row < this.rows; row++) {
                ...
                if (isPlayerTurn && unit) {
                    if (this.state === 'PHASE_MOVE' || this.state === 'MOVE_PREVIEW') {
                        if (this.reachable && this.reachable[`${x},${y}`]) hex.classList.add('movable');
                        ...
                        if (this.getDistance(unit.x, unit.y, x, y) === 1) {
                            ...
                            if (targetUnit && unit.ap >= 1) hex.classList.add('attackable');
                            else {
                                let targetDir = this.getDirection(unit.x, unit.y, x, y);
                                let turnCost = this.getTurnCost(unit.direction, targetDir);
                                if (unit.ap >= turnCost) hex.classList.add('fw-dir-highlight');
                            }
                        }
                        if (x === unit.x && y === unit.y) hex.classList.add('movable');
                    } else if (this.state === 'PHASE_ATTACK') {
                        const targetUnit = this.units.find(u => u.x === x && u.y === y && u.isAttacker !== unit.isAttacker);
                        if (targetUnit && this.getDistance(unit.x, unit.y, x, y) === 1 && unit.ap >= 1) hex.classList.add('attackable');
                        if (x === unit.x && y === unit.y) hex.classList.add('movable');
                    }
                }
    ...
    drawPath(pathArr, startX, startY) {
        ...
        let pts = [];
        const getCenter = (hx, hy) => `${hx * (this.hexW * 0.75) + this.hexW / 2},${hy * (this.hexH / 2) + this.hexH / 2}`;
        pts.push(getCenter(startX, startY));
        for (let p of pathArr) pts.push(getCenter(p.x, p.y));
    ...
    getDistance(x1, y1, x2, y2) { return Math.abs(x1 - x2) + Math.max(0, (Math.abs(y1 - y2) - Math.abs(x1 - x2)) / 2); }

    getNeighbors(x, y) {
        const list = [];
        const dirs = [ {dx: 0, dy: -2}, {dx: 1, dy: -1}, {dx: 1, dy: 1}, {dx: 0, dy: 2}, {dx: -1, dy: 1}, {dx: -1, dy: -1} ];
        for (const d of dirs) {
            const nx = x + d.dx; const ny = y + d.dy;
            if (nx >= 0 && nx < this.cols && ny >= 0 && ny < this.rows * 2) list.push({x: nx, y: ny});
        }
        return list;
    }

    getDirection(fromX, fromY, toX, toY) {
        const dirs = [ {dx: 0, dy: -2, dir: 0}, {dx: 1, dy: -1, dir: 1}, {dx: 1, dy: 1, dir: 2}, {dx: 0, dy: 2, dir: 3}, {dx: -1, dy: 1, dir: 4}, {dx: -1, dy: -1, dir: 5} ];
        for(let d of dirs) { if (toX - fromX === d.dx && toY - fromY === d.dy) return d.dir; }
        return 0;
    }
    ...
    getCost(x, y, enemies, allies, isFirstStep, startDist) {
        ...
        enemies.forEach(e => { let d = this.getDistance(x, y, e.x, e.y); if (d < minEnemyDist) minEnemyDist = d; });
    ...
    findPaths(unit, maxAP) {
        ...
        let visited = {}; visited[`${unit.x},${unit.y}`] = { cost: 0, path: [] };
        ...
        enemies.forEach(e => { let d = this.getDistance(unit.x, unit.y, e.x, e.y); if (d < minStartDist) minStartDist = d; });
        ...
        while(queue.length > 0) {
            ...
            for(let n of neighbors) {
                let c = this.getCost(n.x, n.y, enemies, allies, (cur.steps === 0), minStartDist);
    ...
    isPlayerTurn() { return this.turnQueue.length > 0 && this.turnQueue[0].isPlayer; }

    startTurn() {
        if (!this.active) return;
        this.units.forEach(u => { u.hasActionDone = false; u.ap = u.mobility; });
        this.turnQueue = [...this.units].sort((a, b) => (b.stats.ldr + b.stats.str) - (a.stats.ldr + a.stats.str));
        this.processQueue();
    }
    ...
    processQueue() {
        ...
        if (this.turnQueue.length === 0) {
            this.turnCount++; this.consumeRice();
            if (this.checkEndCondition()) return;
            this.startTurn(); return;
        }
        const unit = this.turnQueue[0];
        if (!this.units.find(u => u.id === unit.id)) { this.nextPhaseTurn(); return; }

        this.turnBackup = { x: unit.x, y: unit.y, direction: unit.direction, ap: unit.ap };
        this.state = 'IDLE'; this.reachable = null; this.previewTarget = null;
        ...
        if (unit.isPlayer) {
            ...
            this.reachable = this.findPaths(unit, unit.ap);
            if (isPlayerInvolved) {
                this.updateMap(); this.updateStatus();
                this.log(`【${unit.name}隊のターン】移動先を選択`);
            }
        } else {
            if (isPlayerInvolved) { this.updateMap(); this.updateStatus(); }
    ...
    nextPhase() {
        const unit = this.turnQueue[0];
        if (this.state === 'PHASE_MOVE' || this.state === 'MOVE_PREVIEW') {
            this.previewTarget = null; this.reachable = null; this.state = 'PHASE_DIR';
            if (unit.ap <= 0) { this.nextPhase(); } 
            else { this.updateMap(); this.updateStatus(); if (this.isPlayerTurn()) this.log(`向き、または攻撃対象を選択`); }
        } else if (this.state === 'PHASE_DIR') {
            this.state = 'PHASE_ATTACK';
            if (unit.ap <= 0) { this.nextPhase(); } 
            else { this.updateMap(); this.updateStatus(); if (this.isPlayerTurn()) this.log(`攻撃対象を選択`); }
        } else if (this.state === 'PHASE_ATTACK') {
            unit.hasActionDone = true; this.state = 'IDLE';
            this.updateMap(); this.updateStatus();
    ...
    consumeRice() {
        ...
        this.atkRice = Math.max(0, this.atkRice - Math.floor(atkSoldiers * consumeRate));
        this.defRice = Math.max(0, this.defRice - Math.floor(defSoldiers * consumeRate));
    }

    checkEndCondition() {
        let atkAlive = false, defAlive = false, atkGeneralAlive = false, defGeneralAlive = false;
        this.units.forEach(u => {
            if (u.isAttacker) { atkAlive = true; if (u.isGeneral) atkGeneralAlive = true; } 
            else { defAlive = true; if (u.isGeneral) defGeneralAlive = true; }
        });
        ...
        if (!atkAlive || !atkGeneralAlive) {
            if (isAtkPlayer) this.log(`総大将が討ち取られ、我が軍は敗北しました……`); else if (isDefPlayer) this.log(`敵の総大将を討ち取りました！`); else this.log(`攻撃軍の総大将が敗走した！`);
            this.endFieldWar('attacker_lose'); return true;
        }
        if (!defAlive || !defGeneralAlive) {
            if (isAtkPlayer) this.log(`敵の総大将を討ち取りました！`); else if (isDefPlayer) this.log(`総大将が討ち取られ、我が軍は敗北しました……`); else this.log(`守備軍の総大将が敗走した！`);
            this.endFieldWar('attacker_win'); return true;
        }
        if (this.atkRice <= 0) {
            if (isAtkPlayer) this.log(`兵糧が尽き、これ以上の行軍は不可能です……`); else if (isDefPlayer) this.log(`${enemyName}は兵糧が尽き、撤退していきました！`); else this.log(`攻撃軍の兵糧が尽き、撤退を余儀なくされた！`);
            this.endFieldWar('attacker_lose'); return true;
        }
        if (this.defRice <= 0) {
            if (isAtkPlayer) this.log(`${enemyName}の兵糧が尽き、城へ敗走していきました！`); else if (isDefPlayer) this.log(`兵糧が底を突き、戦線を維持できません……`); else this.log(`守備軍の兵糧が尽き、城へ敗走した！`);
            this.endFieldWar('attacker_win'); return true;
        }
        if (this.turnCount > this.maxTurns) {
            this.log(`野戦では決着がつかず、舞台は籠城戦へと移る！`);
            this.endFieldWar('draw_to_siege'); return true;
        }
        return false;
    }
    ...
    onHexClick(x, y) {
        ...
        if (this.state === 'PHASE_MOVE') {
            if (x === unit.x && y === unit.y) { this.nextPhase(); return; }
            let key = `${x},${y}`;
            if (this.reachable && this.reachable[key]) {
                this.previewTarget = {x: x, y: y, path: this.reachable[key].path, cost: this.reachable[key].cost};
                this.state = 'MOVE_PREVIEW'; this.updateMap();
            }
        } else if (this.state === 'MOVE_PREVIEW') {
            if (x === this.previewTarget.x && y === this.previewTarget.y) {
                unit.ap -= this.previewTarget.cost; unit.x = x; unit.y = y;
                this.log(`${unit.name}隊が移動。`); this.nextPhase();
            } else {
                let key = `${x},${y}`;
                if (this.reachable && this.reachable[key]) {
                    this.previewTarget = {x: x, y: y, path: this.reachable[key].path, cost: this.reachable[key].cost};
                    this.updateMap();
                } else { this.state = 'PHASE_MOVE'; this.previewTarget = null; this.updateMap(); }
            }
        } else if (this.state === 'PHASE_DIR') {
            if (x === unit.x && y === unit.y) { this.nextPhase(); return; }
            if (this.getDistance(unit.x, unit.y, x, y) === 1) {
                const targetUnit = this.units.find(u => u.x === x && u.y === y && u.isAttacker !== unit.isAttacker);
                if (targetUnit) { if (unit.ap >= 1) { unit.ap -= 1; this.executeAttack(unit, targetUnit); } return; }
                let targetDir = this.getDirection(unit.x, unit.y, x, y);
                let turnCost = this.getTurnCost(unit.direction, targetDir);
                if (unit.ap >= turnCost) {
                    if (turnCost > 0) { unit.ap -= turnCost; unit.direction = targetDir; this.log(`${unit.name}隊が向きを変更。`); }
                    this.nextPhase();
                }
            }
        } else if (this.state === 'PHASE_ATTACK') {
            if (x === unit.x && y === unit.y) { this.nextPhase(); return; }
            const targetUnit = this.units.find(u => u.x === x && u.y === y && u.isAttacker !== unit.isAttacker);
            if (targetUnit && this.getDistance(unit.x, unit.y, x, y) === 1 && unit.ap >= 1) { unit.ap -= 1; this.executeAttack(unit, targetUnit); }
        }
    }
    ...
    getDirectionalMultiplier(atkUnit, defUnit) {
        ...
        let diff = Math.min(Math.abs(defDirIndex - atkDirIndex), 6 - Math.abs(defDirIndex - atkDirIndex)); 
        if (diff === 3) return 1.5; if (diff === 2) return 1.2; return 1.0; 
    }

    executeAttack(attacker, defender) {
        const result = WarSystem.calcWarDamage(
            attacker.stats, defender.stats, attacker.soldiers, defender.soldiers, 0, 
            this.atkMorale, this.defTraining, 'charge'
        );
        ...
        defender.soldiers -= dmgToDef; attacker.soldiers -= dmgToAtk;
        ...
        if (defender.soldiers <= 0) { this.log(`${defender.name}隊が壊滅した！`); this.units = this.units.filter(u => u.id !== defender.id); }
        if (attacker.soldiers <= 0) { this.log(`${attacker.name}隊が壊滅した！`); this.units = this.units.filter(u => u.id !== attacker.id); }
        
        attacker.hasActionDone = true; this.state = 'IDLE';
        this.updateMap(); this.updateStatus();
        setTimeout(() => { this.nextPhaseTurn(); }, 800);
    }
    ...
    async processAITurn() {
        ...
        if (enemies.length === 0) { this.nextPhaseTurn(); return; }

        const isPlayerInvolved = this.units.some(u => u.isPlayer);
        if (isPlayerInvolved) { await new Promise(r => setTimeout(r, 600)); }

        let targetEnemy = null; let minDist = 999;
        enemies.forEach(e => { let d = this.getDistance(unit.x, unit.y, e.x, e.y); if (d < minDist) { minDist = d; targetEnemy = e; } });

        let allySoldiers = 0, enemySoldiers = 0;
        this.units.forEach(u => { if (u.isAttacker === unit.isAttacker) allySoldiers += u.soldiers; else enemySoldiers += u.soldiers; });
        ...
        if (dist > 1) {
            ...
            let bestTarget = null; let minMoveDist = 999;
            for (let key in reachable) {
                let parts = key.split(','); let nx = parseInt(parts[0]); let ny = parseInt(parts[1]);
                let d = this.getDistance(nx, ny, targetEnemy.x, targetEnemy.y);
                if (d < minMoveDist) { minMoveDist = d; bestTarget = {x: nx, y: ny, cost: reachable[key].cost}; }
            }
            
            if (bestTarget && (bestTarget.x !== unit.x || bestTarget.y !== unit.y)) {
                unit.ap -= bestTarget.cost; unit.x = bestTarget.x; unit.y = bestTarget.y;
                if (isPlayerInvolved) { this.log(`${unit.name}隊が前進。`); this.updateMap(); this.updateStatus(); await new Promise(r => setTimeout(r, 400)); }
            }
        }
        ...
        if (dist === 1) {
            ...
            if (unit.ap >= turnCost && turnCost > 0) {
                unit.ap -= turnCost; unit.direction = targetDir;
                if (isPlayerInvolved) { this.log(`${unit.name}隊が敵に向き直った。`); this.updateMap(); this.updateStatus(); await new Promise(r => setTimeout(r, 300)); }
            }
        }

        if (dist === 1 && unit.ap >= 1) { unit.ap -= 1; this.executeAttack(unit, targetEnemy); return; }
