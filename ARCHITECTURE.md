...
- `js/interview_view.js` — 面談専用View。面談開始から終了まで同一モーダルで、面談相手選択・他者選択・会話の順送りを切り替える。一覧超過はスクロールさせずPC12人／スマホ8人単位のページ切替で処理し、名前検索と既知情報（名前・所在・身分・6能力）のソートは `BushoListSortRules` を共用する。PCは16:9、スマホは9:16の固定ゲーム領域へ収める。会話本文は通常会話と同じ `.message-area` を再利用し、余白には面談相手の顔・所在・身分・年齢・6能力だけを表示する。忠誠・義理・野望・相性は「面談で探る情報」なので人物サマリーにもソート候補にも数値表示しない。［戻る］［次へ］［面談を終える］や確認操作は共通UI規約どおり `modal-footer` で内容枠の外側・下へ15px間隔で置き、面談内容そのものの選択肢・ページ送り・並び方向だけは詳細画面系の内側用ボタンを使う。判定・台詞生成は `InterviewSystem` / `PersonnelRules` に委譲する。
