...
- スマホの勢力色・雪・地方/勢力ハイライトなど全画面Canvasは内部解像度を縦横1/2（面積1/4）にし、CSSで地図サイズへ拡大する。描画時も巨大ImageDataを一枚作らず32行程度の帯へ分割する。城・国の判定用IDマップ自体は正確な当たり判定のため原寸を維持する。
...
- 武将一覧・詳細などで背景更新を止める時、スマホでは雪・地方・キープ光など非必須Canvasをいったん解放し、城カードのdrop-shadow/animationも停止する。復帰時は勢力色Canvasの所有城1pixelだけを検査し、低メモリでbacking storeが失われていればhashを無効化して再描画する。Canvasの `contextlost/contextrestored` も同じ復旧経路へ接続する。
...
- 現行データでは `Busho` / `Princess` の生成時と `FamilyLinker` が `baseFamilyIds` / `familyIds` を必ず構築するため、旧セーブ互換だけを目的とした全人物の親族配列再正規化を月初・月末・死亡処理では行わない。
