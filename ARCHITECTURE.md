...
- `js/conversation_standing_rules.js` — 会話上の人物・大名家の格を一元管理する。家中身分、官位、功績、大名家威信、将軍/将軍候補の特殊権威は別軸で保持し、一つの巨大な格スコアへ潰さない。大名家同士は征夷大将軍の別格性→官位の `rankNo` を優先し、同格官位または双方無官の時だけ威信比を会話上の上下へ使う。左馬頭の将軍候補性は人物本人への特別な礼遇に使うが、大名家同士の官位比較そのものを `rankNo` より先に逆転させない。使者は主君の外交上の格を背負いつつ本人の軍師/国主/城主等の身分・官位・功績でも礼遇を微調整する。将軍・将軍候補本人が他家所属になっても公的権威を失わせず、使者時に主君より本人の格式が高い場合は「主君様の名代」として本人を下げない。功績は非公開値のまま、面談等で呼称・敬語・実績を匂わせる表現だけに使い、数値や厳密な段階を露呈させない。
...
- `js/diplomacy.js` — 外交。外交効果・AI判断に加え通常外交会話の生成を担当する。会話上の格そのものは `ConversationStandingRules` を正本とし、プレイヤー発の外交・AIからプレイヤーへの外交・臣従コモンイベントは `buildDiplomacyGreeting()` / `getDiplomacyMessages()` を共用する。使者本人の身分差や官位・功績、大名家同士の官位・威信は成功率へ混ぜず、呼称・敬語・要求/返答の柔らかさだけへ反映する。歴史イベントの演出台詞は作者が意図した個別演出を優先し、自動で上書きしない。
