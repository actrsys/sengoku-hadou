...
- `js/interview_view.js` — 面談専用View。面談開始から終了まで同一モーダル内で、面談相手選択・他者選択・会話の順送りを切り替える。一覧超過はスクロールさせずPC12人／スマホ8人単位のページ切替で処理する。PCは16:9、スマホは9:16の固定ゲーム領域へ収め、判定・台詞生成は `InterviewSystem` / `PersonnelRules` に委譲する。
