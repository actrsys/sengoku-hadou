...
- `js/ai_domestic_priority_rules.js` — AIの石高開発・鉱山開発の基礎スコアと性格／攻撃準備中補正を一元管理する。AI本体と面談の「方針について」は同じ評価式を参照し、面談専用の別スコアを作らない。
...
- `js/busho_list_sort_rules.js` — 武将一覧で共用する既知情報の検索・ソート規則。名前・所在・身分など画面横断で同じ意味を持つ並び順をここへ集約し、敵能力の秘匿・推定値や選択済み優先など画面固有事情は各View側に残す。
...
- `js/gunshi_system.js` — 軍師助言と家臣忠誠の所見を管理する。一般助言の精度は智謀を主軸に軍師自身の忠誠・義理も加味する。褒美一覧と月初忠誠報告は `getLoyaltyAssessment()` を共通の正本とし、実忠誠をUIが直接読んで危険度を表示しない。忠誠所見の可視化は赤（危険）・橙（注意）・無色の3状態だけとし、褒美一覧は赤→橙→無色を優先する。対象武将の智謀による偽装は `LoyaltyInsightRules` と共有し、軍師の智謀が高ければ看破しやすく、軍師自身の忠誠・義理が極端に低い場合は報告が甘くなり得る。軍師不在時は忠誠由来の色分け・優先ソートを行わない。
- `js/interview_view.js` — 面談専用View。面談開始から終了まで同一モーダルで、面談相手選択・他者選択・会話の順送りを切り替える。一覧超過はスクロールさせず、PCは15人、スマホは初回の面談相手選択16人／面談相手サマリーを併記する他者選択12人単位のページ切替で処理し、名前検索と既知情報（名前・所在・身分・6能力）のソートは `BushoListSortRules` を共用する。PCは16:9、スマホは9:16の固定ゲーム領域へ収める。人物サマリーは所在・身分・年齢・6能力を表示するが、能力値は `StatPresenter` のランク表記を使い数値は出さない。忠誠・義理・野望・相性は「面談で探る情報」なのでサマリーにもソート候補にも数値表示しない。顔はサマリーへ重複表示せず、会話時だけ通常会話と同じ「顔＋名前＋ `.message-area`」の一体レイアウトで出す。人物名は顔列の中へ独自配置せず、通常会話と同じく会話枠の上辺へ重ねる。面談モーダル内では顔＋本文をさらに薄い会話外枠で一体化し、選択肢＋会話枠の組をモーダル下部へ固定する。選択肢のない連続会話は標準の［次へ］ボタンを出さず、モーダル内のどこでもクリック/タップ＋▼で順送りする。面談会話の表示時は元の台詞データを変更せず、改行（HTMLのbrを含む）を表示上だけ除去し、閉じ鉤括弧 `」` 直前の句点 `。` も表示上だけ省く。［戻る］［面談を終える］や確認操作は共通UI規約どおり `modal-footer` で内容枠の外側・下へ15px間隔で置き、面談内容そのものの選択肢・ページ送り・並び方向だけは詳細画面系の内側用ボタンを使う。button系SEは共通監視へ任せ、面談内の特殊な選択音は `data-se` で宣言する。武将カードは押下時も位置を動かさず、色・明度・枠だけで反応を示して最下段の見切れを防ぐ。判定・台詞生成は `InterviewSystem` / `PersonnelRules` に委譲する。
- `js/interview_system.js` — 面談の進行・判定・台詞生成の正本。忠誠評価は軍師の警告境界（85以上安定／75～84注意、74以下は危険域）と連動し、危険域以下は60・40・25を境に段階化する。忠誠段階と智謀による偽装自体は `LoyaltyInsightRules` を共用する。本人の自己申告では、智謀70～89なら実際の忠誠評価より1段階、智謀90以上なら2段階だけ高く見せる（上限は安定段階。低忠誠から最高評価へ飛ばさない）。他者評価でも同じ表向き段階を使い、聞き手の智謀・接触度・義理と対象の智謀・野望から情報精度を計算する。高智謀の対象による偽装は、十分な智謀と情報精度を持つ聞き手だけが看破でき、看破時は「表向き」と「本心」を分けた専用台詞を使う。情報精度が極端に低い場合は忠誠を推測せず「内心をほとんど見抜けない」専用台詞へ分岐し、直前の好悪・接触評価を否定しない接続表現にして会話前後の矛盾を避ける。 面談開始時は初回面談による忠誠+1を先に確定してから、現在忠誠・智謀による偽装・義理・野望からその面談中の表面態度を一度だけ決め、最初の挨拶と以後の導入口調へ共通して使う。低忠誠の聞き手に他者評価を頼んだ場合も、自身の不満を固定台詞で自動露呈させず、その面談で確定した表面態度と通常の情報精度判定に従う。観察者視点の『目を合わせない』『危険な気配』などを本人の台詞として出さず、言い淀み・短い返答・過剰な丁寧さなど本人の発話だけで態度を表現する。これらの調整値は `GameConfig.Main.Interview` を正本とし、85/75の境界は `GameConfig.Main.Gunshi` から導出する。 他者の忠誠評価には、相性差1につき1を基礎とする悲観バイアスを加え、革新性の差と聞き手の野望で悪化させる一方、聞き手自身の忠誠・義理・主君との相性が高いほど私情を抑える。情報精度が不足している場合は真の忠誠を漏らさず、偏見が強い場合だけ根拠の薄い警戒として表現する。『方針について』は軍団長に対してのみ軍団の具体方針を聞け、攻撃は現在のAI作戦第一目標、内政は `AIDomesticPriorityRules` の軍団内最高スコアを根拠にする。開示量は忠誠・義理で決め、面談専用AI判断は作らない。
