...
- `js/interview_system.js` — 面談の進行・判定・台詞生成の正本。忠誠評価は軍師の警告境界（85以上安定／75～84注意、74以下は危険域）と連動し、危険域以下は60・40・25を境に段階化する。本人の自己申告では、智謀70～89なら実際の忠誠評価より1段階、智謀90以上なら2段階だけ高く見せる忠誠偽装を使う（上限は安定段階。低忠誠から最高評価へ飛ばさない）。他者評価でも同じ表向き段階を使い、聞き手の智謀・接触度・義理と対象の智謀・野望から情報精度を計算する。高智謀の対象による偽装は、十分な智謀と情報精度を持つ聞き手だけが看破でき、看破時は「表向き」と「本心」を分けた専用台詞を使う。情報精度が極端に低い場合は忠誠を推測せず「内心をほとんど見抜けない」専用台詞へ分岐し、直前の好悪・接触評価を否定しない接続表現にして会話前後の矛盾を避ける。 面談開始時は実忠誠・智謀による偽装・義理・野望からその面談中の表面態度を一度だけ決め、最初の挨拶と以後の導入口調へ共通して使う。観察者視点の『目を合わせない』『危険な気配』などを本人の台詞として出さず、言い淀み・短い返答・過剰な丁寧さなど本人の発話だけで態度を表現する。これらの調整値は `GameConfig.Main.Interview` を正本とし、85/75の境界は `GameConfig.Main.Gunshi` から導出する。
