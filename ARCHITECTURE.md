...
- **固定モーダルでは状態切替で外形を動かさない。** 選択肢の有無や外側フッターの有無で内容枠の位置・高さを変えず、必要な操作スロットは固定領域として予約する。面談の会話選択肢は会話本文より上に置く。
# 戦国覇道 コード構成ガイド
...
- `js/interview_view.js` — 面談専用View。面談開始から終了まで同一モーダルで、面談相手選択・他者選択・会話の順送りを切り替える。一覧超過はスクロールさせずPC12人／スマホ8人単位のページ切替で処理し、名前検索と既知情報（名前・所在・身分・6能力）のソートは `BushoListSortRules` を共用する。PCは16:9、スマホは9:16の固定ゲーム領域へ収める。人物サマリーは所在・身分・年齢・6能力を表示するが、能力値は `StatPresenter` のランク表記を使い数値は出さない。忠誠・義理・野望・相性は「面談で探る情報」なのでサマリーにもソート候補にも数値表示しない。顔はサマリーへ重複表示せず、会話時だけ通常会話と同じ「顔＋名前＋ `.message-area`」の一体レイアウトで出す。選択肢のない連続会話は標準の［次へ］ボタンを出さず、モーダル内のどこでもクリック/タップ＋▼で順送りする。［戻る］［面談を終える］や確認操作は共通UI規約どおり `modal-footer` で内容枠の外側・下へ15px間隔で置き、面談内容そのものの選択肢・ページ送り・並び方向だけは詳細画面系の内側用ボタンを使う。button系SEは共通監視へ任せ、面談内の特殊な選択音は `data-se` で宣言する。判定・台詞生成は `InterviewSystem` / `PersonnelRules` に委譲する。
